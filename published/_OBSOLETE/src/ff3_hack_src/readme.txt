...
まず以下の二つを用意する必要があります
	[1] nesasm (http://www.magicengine.com/mkit/)
	[2] iNESヘッダ(先頭10hバイト)を削ったパッチをあてたいff3

ソース一式を展開したフォルダに[2]を ff3_hack_base.nes という名前で置き
nesasmで以下のいずれかのバージョンをアセンブルすれば
	ff3_hack.asm
	ff3_hack_beta.asm
...
同じフォルダに <アセンブルしたファイル>.nes という名前でパッチが適用されたff3が出来上がります
...
[ファイルの説明など (ff3_hack_master.asm) ]

パッチに実装されている機能ごとにおおまかにわけてありますが
オリジナルのコードを最適化して出来たスペースに
(元の機能と全く関係の無い)新しいコードを埋め込むという形で実装されているものも多いため
...
ENABLE_EXTRA_ABILITY_TAG
	パッシブ能力と拡張コマンド(ちょうはつなど)の実装を有効にします
