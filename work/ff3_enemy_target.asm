...
.for_each_enemy:
	...
	.check_status_2nd:
		iny
		lda [.pEnemy],y
		and #STATUS_CONFUSED	;$20
		bne .status_confused
		and #STATUS_SLEEPING | STATUS_PARALYZED	;$c0
		beq .status_good
	;paralyzed|sleeping
			lda #1
			jmp .next		;a8b9
	.status_confused:
		lda #$80
		sta <.confused
		bne .select_action	;always satisfied
	.status_good:
		lda .battleMode
...
.lv = $28
	...
	.select_action:	;$a7c5
		ldx <.iEnemy
		lda .indexToGroupMap,x	;2007-08-21
		tax
	...
	.check_barrier_change:
			...
			cmp #2
			bne .check_special
				jsr getActionMode	;$a8c8
				ora #$10
				sta [.pEnemy],y
				lda #$4d	;4d:barrier-change
				jmp .next
	.check_special:	;a7e9
		ldy #BP_OFFSET_SPECIAL_RATE	;$37
		lda [.pEnemy],y
		beq .decide_to_fight
			lda #100
	...
	.decide_to_fight:	;a7fd
		;new decideEnemyTarget implementation has proper handling for confused enemy
	.select_fight_target:
		;select valid target (must not be dead/stone/jumping)
		;if there is no valid target,then decide to action0 (that do nothing)
		lda #ACTION_FIGHT	;4
		;pha
		sta <$46
		lda #0
		jsr decideEnemyTarget	;[out] a = target bit
		beq .fail_to_target
		;pla
		lda #ACTION_FIGHT
		bne .next
	.select_special:	;$a85f
		jsr getActionMode	;$a8c8	;get24_2c
		ora #$10
		sta [.pEnemy],y
		lda .battleMode
		bpl .select_randomly
	...
	.fail_to_target:
			...
			beq .next
	.setTarget:
	;a = offset to selected command (in list)
	;[out] a : = $69, x : decided command
			tay
			lda [.pEnemy],y
			;and #$7f
			;pha
			lda [.pEnemy],y
			pha
			jsr decideEnemyActionTarget	;a91b [out] $69 : failed
			pla
			and #$7f
			tax
			lda <$69	;failed flag
...
.select_from_enemy_party:
		...
		ldx #$ff
		pla
		and #TARGET_MULTIPLE	;$40
		ora #TARGET_ENEMY_PARTY	;$80
		bmi .set_target
...
.target_single:
	.ifdef TAG_PROVOKE_EFFECT
		ldy #X_BP_OFFSET_TYPE	;$1f	;enchanted status(lefthand)
		lda [.pEnemy],y
		and #$1f
		beq .do_usual
			sec
